(function ($) {
    ...
    var footLabels = ['去倒垃圾', '逃過一劫', 'ㄏㄏ不用啦', 'QQ可惜不用', 'E', 'F', 'G', 'H', 'I', 'J'];
    ...
    var loadMenu = function (root) {
        ...
        var shuffleButton = $('<a>換圖</a>').attr('data-role', 'button')
            .attr('data-theme', 'c')
            .attr('id', '__ak_shuffle_button')
            .appendTo(menu);
        var editButton = $('<a>編輯按鈕(可以打名字)</a>').attr('data-role', 'button')
            .attr('data-theme', 'c')
            .attr('id', '__ak_edit_button')
            .attr('data-icon', 'gear')
            .appendTo(menu);
        var saveButton = $('<a>儲存</a>').attr('data-role', 'button')
